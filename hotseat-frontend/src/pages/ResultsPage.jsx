...
export default function ResultsPage() {
  ...
  return (
    <div className="w-full h-full flex flex-col lg:flex-row lg:overflow-hidden">

      {/* ───── MAIN AREA: Question + Answers ───── */}
      <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar lg:pr-6">
        {/* Header row */}
        <div className="flex justify-between items-center pb-3 flex-shrink-0 sticky top-0 z-10 bg-[#050508]/80 backdrop-blur-sm">
        ...
        <motion.div
          ...
          className="bg-zinc-900/60 border border-white/10 backdrop-blur-2xl rounded-3xl p-5 md:p-7 shadow-xl relative overflow-hidden group"
        >
        ...
        {/* Mobile-only compact chat (visible on small screens) */}
        <div className="mt-6 lg:hidden">
          <ChatPanel
            messages={messages} user={user} typingUsers={typingUsers}
            showGifPicker={showGifPicker} isSearchingGifs={isSearchingGifs}
            gifQuery={gifQuery} gifs={gifs}
            messageInput={messageInput}
            isUploading={isUploading}
            chatScrollRef={chatScrollRef} chatEndRef={chatEndRef}
            fileInputRef={fileInputRef}
            text={text} SERVER_URL={SERVER_URL}
            onTyping={handleTyping} onSendText={handleSendText}
            onSendReaction={handleSendReaction}
            onGifQueryChange={handleGifQueryChange}
            onSendGif={handleSendGif}
            onImageUpload={handleImageUpload}
            onToggleGif={() => { playSFX('click'); setShowGifPicker(p => !p) }}
          />
        </div>
      </div>

      {/* ───── DESKTOP RIGHT SIDEBAR: Live Chat ───── */}
      <div className="hidden lg:flex lg:w-[420px] lg:border-l lg:border-white/10 lg:h-full lg:overflow-hidden">
        <ChatPanel
          messages={messages} user={user} typingUsers={typingUsers}
          showGifPicker={showGifPicker} isSearchingGifs={isSearchingGifs}
          gifQuery={gifQuery} gifs={gifs}
...
// ───── Shared Chat Panel (used by both mobile & desktop) ─────
function ChatPanel({
  messages, user, typingUsers,
  showGifPicker, isSearchingGifs, gifQuery, gifs,
  messageInput, isUploading,
  chatScrollRef, chatEndRef, fileInputRef,
  text, SERVER_URL,
  onTyping, onSendText, onSendReaction,
  onGifQueryChange, onSendGif, onImageUpload, onToggleGif,
}) {
  return (
    <div className="flex flex-col bg-zinc-900/40 border border-white/10 lg:border-0 backdrop-blur-2xl rounded-3xl lg:rounded-none p-5 md:p-6 h-full">
      {/* Chat header */}
        ...
        {showGifPicker && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 16 }} transition={{ duration: 0.2 }}
            ...
            <div className="flex items-center gap-2 mb-3">
              <input value={gifQuery} onChange={e => onGifQueryChange(e.target.value)} placeholder={text.searchGifs ?? 'Search GIFs…'} autoFocus className="flex-1 bg-zinc-800/80 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white font-bold outline-none focus:border-white/30 transition-colors" />
              <button onClick={() => { /* close handled by parent */ }} className="w-9 h-9 flex items-center justify-center rounded-full bg-zinc-800/80 border border-white/10 text-zinc-400 hover:text-white transition-all"><X size={16} /></button>
            </div>
            <div className="h-36 overflow-y-auto grid grid-cols-3 gap-2">
              ...
              : gifs.map(gif => <button key={gif.id} onClick={() => onSendGif(gif.images.fixed_height.url)} className="rounded-xl overflow-hidden h-20 w-full bg-zinc-800/80 border border-white/10 active:scale-95 transition-all"><img src={gif.images.fixed_height.url} alt="gif" className="w-full h-full object-cover" /></button>)}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="relative flex items-center bg-zinc-950/90 border border-white/10 focus-within:border-white/30 rounded-full p-2 pl-5 shadow-2xl transition-all flex-shrink-0">
        ...
        <button onClick={onToggleGif} className={`p-1.5 transition-colors shrink-0 ${showGifPicker ? 'text-white' : 'text-zinc-400 hover:text-white'}`}>
          <Smile size={18} />
        </button>
        <textarea
          value={messageInput}
